...
class Patchifier:
    ...
    def __call__(self, img):
        """ """
        B, F, C, H, W = img.shape # [Batch, FrameNumbers, Channels, Height, Width]
        ...
        patch_data = img.reshape(B, F, C, num_patch_H, self.patch_size, num_patch_W, self.patch_size)
        patch_data = patch_data.permute(0, 1 ,3, 5, 2, 4, 6)
        patch_data = patch_data.reshape(B, F, num_patch_H * num_patch_W, C * self.patch_size * self.patch_size)
        return patch_data #(BatchSize, Frames, num_patch_H * num_patch_W, C * self.patch_size * self.patch_size)
